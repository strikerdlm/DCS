...
def monte_carlo_prediction(models, X, n_samples=1000, uncertainty=0.05):
    ...
    for _ in tqdm(range(n_samples)):
        ...
        for i, model in enumerate(models):
            model_preds[:, i] = model.predict(X_noisy)
    ...
    mean_pred = np.mean(predictions, axis=0)  # Shape: (n_instances,)
    std_pred = np.std(predictions, axis=0)    # Shape: (n_instances,)
    lower_ci = np.percentile(predictions, 2.5, axis=0)  # Shape: (n_instances,)
    upper_ci = np.percentile(predictions, 97.5, axis=0) # Shape: (n_instances,)
