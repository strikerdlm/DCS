...
def plot_risk_of_decompression_sickness(time_at_altitude, risk_prediction):
    output_data_df = pd.read_excel(r"C:\Users\User\OneDrive\FAC\Research\DCS FAC\output_data_set.xlsx")

    time_data = output_data_df['time_at_altitude']
    decompression_risk_data = output_data_df['calculated']

    popt, _ = curve_fit(func, time_data, decompression_risk_data)
    a, b = popt

    x_line = np.arange(0, 241, 1)
    ...
    # Highlight the result in a red box
    plt.axhline(y=risk_prediction, color='r', linestyle='--', label='Predicted Risk')
    plt.axvline(x=time_at_altitude, color='r', linestyle='--', label='Time at Altitude')
