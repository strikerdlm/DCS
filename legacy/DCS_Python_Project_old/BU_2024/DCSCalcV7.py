...
def plot_risk_of_decompression_sickness(time_at_altitude, risk_prediction):
    # Load data from the output_data_set Excel file
    output_data_file_path = r"C:\Users\User\OneDrive\FAC\Research\DCS FAC\output_data_set.xlsx"
    output_data_df = pd.read_excel(output_data_file_path)

    # Extract required columns
    time_at_altitude_data = output_data_df['time_at_altitude']
    decompression_risk_data = output_data_df['calculated']

    # Plot the data
    plt.plot(time_at_altitude_data, decompression_risk_data, label='Predicted Decompression Risk')

    # Highlight the intercept
    plt.axvline(time_at_altitude, 0, risk_prediction, color='red', linewidth=1, alpha=0.5)

    # Set axis labels and title
    plt.xlabel('Time at Altitude (min)')
    plt.ylabel('Predicted Risk of Decompression Sickness (%)')
    plt.title('Risk of Decompression Sickness vs. Time at Altitude')
...
# Get user input
    ...
    _, _, time_at_altitude, _ = user_input  # Extract time_at_altitude from user_input
...
# Create the graph
    plot_risk_of_decompression_sickness(time_at_altitude, risk_prediction)
